"""Run slope sensitivity for MICE-PMM imputation across synthetic dataset sizes."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run slope sensitivity for MICE-PMM imputation."
    )
    parser.add_argument(
        "--synthetic-size",
    ...
    parser.add_argument(
        "--m",
        type=int,
        default=MICE_N_IMPUTATIONS,
        help="MICE-PMM imputations per missing dataset.",
    )
    parser.add_argument(
        "--output-prefix",
        default="sensitivity_mice_pmm_slope4",
        help="Short output stem for this method.",
    )
    cli_args = parser.parse_args()
    cli_args.methods = DEFAULT_METHODS
    cli_args.base_seeds = DEFAULT_SEEDS
    cli_args.m = cli_args.m
    return cli_args
