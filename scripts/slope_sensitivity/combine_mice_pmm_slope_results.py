"""Combine the four split-slope MICE-PMM sensitivity results."""
...
OUTPUT_PREFIX = "sensitivity_mice_pmm_slope4"
...
def load_split_results(results_dir: Path, allow_incomplete: bool) -> pd.DataFrame:
    ...
    if not frames:
        raise FileNotFoundError(f"No split MICE-PMM result CSVs found in {results_dir}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Combine the four split-slope MICE-PMM sensitivity results."
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
