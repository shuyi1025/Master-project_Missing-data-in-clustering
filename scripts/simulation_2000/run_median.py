...
def load_synthetic_complete_data():
    ...
    return x_complete, true_labels, benchmark_labels
...
def impute_median(x_missing, random_state):
...
def run(args):
    ...
    for b in range(1, b_repetitions + 1):
        for mechanism in mechanisms:
            for rate in rates:
                ...
                method_result = impute_median(
                    x_missing,
                    random_state=seed("method", b, int(rate * 1000), mechanism, METHOD),
                )
                ...
                for m, x_completed in enumerate(method_result["completed_datasets"], start=1):
                    model, labels = run_kmeans(
                        x_completed,
                        random_state=seed("kmeans", b, int(rate * 1000), mechanism, METHOD, m),
                    )
                    metric_rows.append(
                        compute_metrics(
                            x_completed,
    ...
    summary_path = summarise_results(results)
