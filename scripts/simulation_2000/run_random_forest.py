...
def summarise_results(results):
    ...
    mechanism_suffix = f"_{mechanisms[0].lower()}" if len(mechanisms) == 1 else ""
    summary_path = OUTPUT_DIR / f"simulation_summary_{METHOD}{mechanism_suffix}.csv"
    summary.to_csv(summary_path, index=False)
    for mechanism, group in summary.groupby("mechanism"):
        group = group.sort_values("target_missing_rate")
        fig, ax = plt.subplots(figsize=(7, 4))
        ax.plot(group["target_missing_rate"] * 100, group["mean_ari"], marker="o")
        ax.set_title(f"{METHOD}: ARI by missingness rate ({mechanism})")
        ax.set_xlabel("Missingness rate (%)")
        ax.set_ylabel("Mean ARI vs full-data K-means")
        ax.set_ylim(0, 1)
        ax.grid(axis="y", alpha=0.3)
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / f"ari_by_rate_{METHOD}_{mechanism.lower()}.png", dpi=220)
        plt.close(fig)
    return summary_path
    ...
    for b in range(1, b_repetitions + 1):
        for mechanism in mechanisms:
            for rate in rates:
                ...
                method_result = impute_random_forest(
                    x_missing,
...
def parse_single_mechanism_args(mechanism):
    parser = argparse.ArgumentParser(
    ...
    if args.output is None:
        ...
        args.output = (
            f"simulation_results_{METHOD}_{mechanism.lower()}{smoke_suffix}.csv"
        )
    return args
...
def run_single_mechanism(mechanism):
    if mechanism not in MISSINGNESS_MECHANISMS:
        raise ValueError(f"Unknown missingness mechanism: {mechanism}")
    run_and_report(parse_single_mechanism_args(mechanism))


if __name__ == "__main__":
    run_and_report(parse_args())
