...
def summarise_results(results):
    ...
    summary_path = OUTPUT_DIR / f"simulation_summary_{METHOD}_mnar.csv"
...
def run(args):
    ...
    output_name = args.output or f"simulation_results_{METHOD}_mnar.csv"
    if args.smoke:
        rates = args.rates or [0.10]
        b_repetitions = args.b or 1
        output_name = args.output or f"simulation_results_{METHOD}_mnar_smoke.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the complete MICE PMM simulation script for MNAR.")
